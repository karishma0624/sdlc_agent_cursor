...
def execute_frontend_phase(planning_content: str, run_dir: str) -> tuple[bool, str]:
    ...
    prompt = (
        ...
        "   - package.json (with react, react-dom, vite, tailwindcss, autoprefixer, postcss, lucide-react)\n"
        ...
        "   - Use best practices in modern web design (e.g. vibrant colors, modern typography, glassmorphism, dark modes).\n"
        "   - Incorporate `lucide-react` icons extensively for a polished UI.\n"
        "   - The UI MUST look extremely premium and WOW the user at first glance. Avoid simple or minimal viable products.\n"
        "   - Use proper layout spacing, shadow effects, and flex/grid. Avoid generic red/blue defaults.\n"
        "   - Create realistic mock data or functional components to fill the UI (NO empty states or placeholders).\n"
        "6. **NO FAKE IMPORTS (CRITICAL)**:\n"
        "   - Do NOT import from '@heroicons/react', 'react-router-dom', 'framer-motion', 'recharts', or any library not explicitly in the package.json provided.\n"
        "   - Do NOT try to import other local components that you do not generate in the JSON. If it's used, you MUST generate the code for it in the JSON.\n"
        "7. **Output**: ONLY the JSON object. NO markdown code fences. NO explanations.\n\n"
