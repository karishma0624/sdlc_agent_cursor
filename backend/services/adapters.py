...
class InferenceRouter:
    ...
    def generate_code(self, instruction: str, preference: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        default_pref = ["gemini", "perplexity", "hf", "mistral", "groq", "openai"]
        result = self.generate_text(prompt, preference=preference or default_pref)  # priority enforced
        ...
        try:
            ...
            # Flexible regex: allows space/newline after ```json, captures content until ```
            m = re.search(r"```(?:json)?\s*([\s\S]*?)```", text)
            if m:
                try:
                    files = _json.loads(m.group(1))
                except:
                    # Try finding the first { if model added text
                    content = m.group(1)
                    if "{" in content:
                        try:
                             start = content.index("{")
                             end = content.rindex("}") + 1
                             files = _json.loads(content[start:end])
                        except: pass
        result["files"] = files
        return result
